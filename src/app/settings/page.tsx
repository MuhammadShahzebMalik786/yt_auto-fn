...
export default function Settings() {
  ...
  useEffect(() => {
    ...
    fetch(`http://localhost:8000/settings?workspace_id=${activeWorkspaceId}`)
      .then(res => res.json())
      .then(data => {
    ...
    // Fetch auth status
    fetch(`http://localhost:8000/auth/status?workspace_id=${activeWorkspaceId}`)
      .then(res => res.json())
      .then(data => {
    ...
    fetch("http://localhost:8000/chrome_profiles")
      .then(res => res.json())
      .then(data => {
  ...
  useEffect(() => {
    fetch(`http://localhost:8000/tts/voices?engine=${ttsEngine}`)
      .then(res => res.json())
  ...
  const handleSave = async () => {
    ...
    setMessage("");
    try {
      const res = await fetch(`http://localhost:8000/settings?workspace_id=${activeWorkspaceId}`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          ai_model: model,
      ...
      const profileResponse = await fetch("http://localhost:8000/chrome_profiles", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const handleDisconnect = async () => {
    try {
      const res = await fetch(`http://localhost:8000/auth/logout?workspace_id=${activeWorkspaceId}`, { method: "DELETE" });
        ...
        <div className="mb-8 pt-8 border-t border-white/10 animate-fade-in-up" style={{ animationDelay: '500ms' }}>
          ...
          <div className="transition-all duration-500">
            {youtubeConnected ? (
              ...
              <a 
                href={`http://localhost:8000/auth/login?workspace_id=${activeWorkspaceId}`}
